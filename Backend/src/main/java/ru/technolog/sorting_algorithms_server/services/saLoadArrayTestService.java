...
import ru.technolog.sorting_algorithms_server.repository.saArrayDataTestRepository;
...
@Service
public class saLoadArrayTestService {
    // Автоматическое внедрение зависимости репозитория для работы с данными массивов
    @Autowired
    private saArrayDataTestRepository arrayDataRepository;
    ...
    public ResponseEntity<ApiResponse> addArrayWithSorting(dtoArray arrayDTO) {
        ...
        try {
            ...
            saArraysData_test saArraysData = new saArraysData_test();
    ...
    public ResponseEntity<ApiResponse> deleteByIdArray(Long arrayId) {
        ...
        Optional<saArraysData_test> saArraysDataOptional = arrayDataRepository.findById(arrayId);
    ...
    public List<dtoArray> getAllArrays() {
        ...
        List<saArraysData_test> arraysList = StreamSupport.stream(arrayDataRepository.findAll().spliterator(), true).toList();
    ...
    public dtoArray mapToArraysDTO(saArraysData_test saArraysData) {
    ...
    public ResponseEntity<ApiResponse> updateArrayWithSorted(Long arrayId, dtoArray arrayDTO) {
        Optional<saArraysData_test> saArraysDataOptional = arrayDataRepository.findById(arrayId);
        if (saArraysDataOptional.isPresent()) {
            saArraysData_test saArraysData = saArraysDataOptional.get();
            saArraysData.setArrayData(arrayDTO.getArrayData());
            saArraysData.setArrayData_sort(arrayDTO.getArrayData_sort());
